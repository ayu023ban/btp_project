...
from configuration import max_vel,   no_of_sensors, range_res, vel_res, max_range, no_of_channels
import signal
import torch
signal.signal(signal.SIGINT, signal.SIG_DFL)


def visualize_baseband_output_per_sample_and_channel(sensor, channel, ax, complex=False):
...
def heat_map(input_sensors, output_sensors):
    for index in range(no_of_sensors):
        ...
        for channel in range(no_of_channels):
            ...
            visualize_baseband_output_per_sample_and_channel(input_sensor, channel, ax1)
            ...
            visualize_baseband_output_per_sample_and_channel(output_sensor, channel, ax2)
